...
void HelloWorld::loadTerrain(int type){
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 2; j++) {
            this->addNewSpriteWithCoords( ccp(0 + 640*i, 0 + 960*j),terrainTag);
        }
    }
}
...
bool HelloWorld::ccTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
    ...
    //judge if tap in exsit resource
    CCARRAY_FOREACH(itemArray,obj){
        CCSprite * resource = (CCSprite*)obj;
        if (resource->boundingBox().containsPoint(pTouch->getLocation())) {
            dragingResource = true;
            dragTag = resource->getTag();
            return true;
        }
    }
    return true;
